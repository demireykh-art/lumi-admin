...
| **ISSUE-8** | 입고 탭이 `totalStock` 만 읽어 대부분 품목 현재고가 0 으로 표시 | 중간 | **HOTFIX-4 (대기)** | 수정 준비됨 |
...
## ISSUE-8 — 입고 탭이 대부분 품목의 현재고를 0 으로 표시

**심각도: 중간 (직원이 입고 시 현재 재고를 확인할 수 없음)**
**처리: HOTFIX-4 — `claude/hotfix-4-receiving-stock-display` 브랜치에 수정 준비됨. 머지 대기**

### 현상

입고 탭이 `item.totalStock` 만 읽습니다.

| 위치 | 코드 |
|---|---|
| `staff.html:15678` | `const stock=item.totalStock\|\|0;` — 품목 검색 결과의 "현재고" |
| `staff.html:15703` | `textContent=(item.totalStock\|\|0)+' '+unit` — 선택 후 "현재고" 표시 |

`totalStock` 은 일부 갱신 경로만 쓰는 그림자 필드라 대다수 문서에 아예 없고,
없으면 `||0` 이 걸려 **0 으로 표시**됩니다.

### 실측 (2026-08-22 프로덕션)

- 전체 **207 품목 중 178 품목(86%)에 `totalStock` 이 없습니다.**
- 같은 조사에서 **`currentStock` 불일치는 0 건** — `currentStock` 이 정본입니다.
- 즉 실재고 525 개인 품목이 입고 탭에서는 "현재고 0" 으로 보입니다.
...
`||` 가 아니라 `??` 입니다 — 재고가 정말 0 인 품목을 걸러내면 안 됩니다.
표시 전용이며 쓰기 경로는 건드리지 않습니다.

### 관련

- 근본 원인은 ISSUE-7(중복 필드)입니다. 데이터에 `totalStock` 을 178 개 채우는 대신
  코드가 정본을 읽게 하는 쪽을 택했습니다 — 중복 필드를 확산시키지 않기 위해서입니다.
- **`DECISIONS-2.md` 결정 A 재검토 필요**: "신규 병원에는 입고 탭 경로만 노출" 로
  정하셨는데, 그 탭은 프로덕션에서 **실사용 기록이 1 건**(207 품목 중 1 품목)뿐입니다.
  사실상 미검증 경로를 신규 병원 기본값으로 삼는 셈이라 재판단이 필요합니다.
