...
- **증상:** 입고 탭이 `item.totalStock` 만 읽었습니다. `totalStock` 은 일부 갱신 경로만
  쓰는 그림자 필드라 대다수 문서에 없고, 없으면 `||0` 이 걸려 현재고가 0 으로 보였습니다.
  실재고 525 개인 품목도 "현재고 0" 으로 표시됐습니다.
- **실측:** 207 품목 중 **178 품목(86%)** 에 `totalStock` 이 없었습니다.
- **수정:** `item.currentStock ?? item.totalStock ?? 0` — `currentStock` 이 정본이므로
  그것을 우선 읽습니다. `||` 가 아니라 `??` 인 이유는 재고가 정말 0 인 품목을
  걸러내면 안 되기 때문입니다.
- **데이터를 고치지 않고 코드를 고친 이유:** 178 개 문서에 `totalStock` 을 채우면
  ISSUE-7(중복 필드)을 확산시키고 신규 품목은 또 0 으로 보입니다.
...
- **과거 데이터: 복구할 것이 없었습니다.** 2026-08-22 프로덕션 조사(`report`) 결과
  207 품목 중 **`currentStock` 불일치 0 건**. 데이터 오염이 실제로는 발생하지 않았습니다.
  - 이유: **월 2회 실사가 자동으로 치유해왔습니다.** 실사(`onInvInput`)는
    `currentStock = Σlocations` 로 덮어쓰므로, 입고 탭 버그로 어긋나도 다음 실사에서
    리셋됩니다. 손상 창이 최대 한 실사 주기(2주)였고 지금은 잔존분이 없습니다.
  - 따라서 **`apply` 는 실행하지 않았습니다.** 스크립트는 그대로 두되 이 건으로는 쓰지 않습니다.
- **근본 원인:** ISSUE-7 (중복 필드 구조)

### FIXED-3 — 발주 예측이 `receivingHistory` 입고를 누락 — **착수하지 않음 (불필요)**

- **판정 (2026-08-22):** HOTFIX-3 **취소.** 프로덕션 조사 결과
  **입고 탭 사용 흔적(`batches[]` 보유)이 207 품목 중 1 품목**뿐입니다(레스노베 앰플, 1건).
  `receivingHistory` 경로가 실질적으로 쓰이지 않으므로 발주 예측에 합산해도 얻는 것이 없습니다.
- 나중에 입고 탭 사용이 늘면 그때 다시 판단합니다. 코드 위치와 방침은 아래에 남겨 둡니다.
...
## 재고 관련 제품 결정 (2026-08-22, HOTFIX 조사 중 확정)

`DECISIONS.md` / `DECISIONS-2.md` 를 보완하는 결정입니다. 충돌하면 이 절이 우선합니다.

### 원칙 — 타겟은 직원 10인 전후 소규모 병원

기능을 넣을지 판단하는 기준입니다. **"자산 전체를 잡아야 하는" 기능은 넣지 않습니다.**
10 인 규모에서 관리 비용이 편익을 넘는 순간 그 기능은 안 쓰이게 됩니다.

### 결정 1 — 「개인 사무용품」은 수량 관리 대상이 아닙니다

무전기·컴퓨터·책상·받침대처럼 **가끔 사는 개인 지급 물품**에 대한 결정입니다.
...
**사유:** 개인별 배정까지 추적하면 자산관리 모듈이 되고, 그건 소규모 병원이 감당하지
않습니다. 실제 불편은 *"오랜만에 주문할 때 어디서 사야 하는지 한참 찾는 것"* 하나입니다.

> 앞서 검토했던 `inventory.type` 에 `fixture`(집기) 추가 안은 **폐기**합니다.
> 수량을 재지 않기로 했으므로 `inventory` 에 넣을 이유가 없습니다.

### 결정 2 — 새로 만들 기능이 없습니다. 기존 「비품 구입」 카탈로그를 씁니다

조사 결과 필요한 기능이 **이미 전부 있습니다.**

| 기존 기능 | 저장소 | 하는 일 |
|---|---|---|
| 🛍️ 비품 구입 | `supplies-catalog.js` (내장 91 항목) + `suppliesCatalogCustom` | **구매처 링크 모음.** 팀별 그룹핑, 구매 횟수·주기 표시 |
| 🛒 구매 요청함 | `purchaseRequests` | 요청 → 승인 → 주문 → 수령 |

카탈로그 항목 구조: `{name, team, link, purchase_count, last_purchase, avg_interval_days}`
— 내장 91 항목 중 85 개에 이미 링크가 있습니다.

**필요한 변경은 분류 하나 추가뿐입니다.**

| 위치 | 변경 |
|---|---|
| `staff.html:1799-1803` | `<select id="catalogAddTeam">` 에 `<option>개인 사무용품</option>` 추가 |
| `staff.html:13989` | `teamOrder` 배열에 `'개인 사무용품'` 추가 (정렬 위치용, 없어도 동작함) |

정렬 로직이 모르는 team 을 이미 맨 뒤로 보내므로(`staff.html:13996-14000`),
**옵션을 추가하지 않아도 "공용" 으로 지금 당장 쓸 수 있습니다.**

**처리 시점: Phase 3.** 이 두 곳은 Phase 3 에서 어차피 설정값(`settings/org`)으로
빼는 하드코딩 enum 입니다. 지금 하드코딩을 하나 더 늘리는 대신 그때 설정값으로 넣습니다.
급하면 2 줄이므로 언제든 앞당길 수 있습니다.

### 결정 3 — `DECISIONS-2.md` 결정 A 부분 수정

> "신규 병원: 입고 탭(`receivingHistory`) 경로만 노출"

**근거가 무너졌습니다.**

- 그 경로는 프로덕션 실사용 기록이 **207 품목 중 1 품목**뿐입니다 (사실상 미검증)
- 신규 병원의 첫 동작은 **입고가 아니라 "이미 쓰고 있는 제품을 등록"** 입니다.
  재고앱을 도입하고 나서 입고가 시작되는 것이 아니라, 도입 시점에 이미 재고가 있습니다.
  즉 온보딩의 첫 화면은 **초기 실사**여야 합니다.

**단, 신규 병원의 기본 진입 모드를 무엇으로 할지는 보류합니다.** 위 두 근거만 기록해
두고, Phase 5(온보딩) 설계 시 다시 결정합니다.

### 남은 데이터 정리 (급하지 않음)

`inventory` 에 **무전기 / 무전기 이어마이크** 2 건이 장소 미지정으로 들어가 있습니다.
소모품이 아니므로 애초에 여기 있을 항목이 아닙니다 — 위 구멍의 증상입니다.
...
### ✅ 결정 (2026-08-22) — **(b) 버림. 손익계산서만 재설계 대기**

원장 방침: *"루미 자산 중에 손익계산서 만드는 것은 다시 업데이트해야 될 것 같다. 그래서
기존에 관리자앱에서의 방식은 없어도 될 것 같다. 나머지 기능도 거의 쓰지 않는다."*

| 기능 | 결정 |
|---|---|
| 고정비 · 변동비 · 카드명세서 · 비용 자동분류 | **폐기** (거의 안 씀) |
| 소득세 · 부가세 · 원천세 | **폐기** (거의 안 씀) |
| `config` (비용 카테고리 · 관리자앱 비밀번호) | **폐기** |
| `recipes` (레시피/시술 원가) | **폐기** |
| **손익계산서(P&L)** | **기존 방식 폐기. 통합앱에서 새로 설계** ← 폐기가 아니라 **재설계 대기** |

### 폐기해도 통합앱은 무영향 — 검증 완료

폐기 대상 9 개 컬렉션에 대해 `staff.html` 의 접근 횟수를 전수 확인했습니다.

| 컬렉션 | 통합앱 접근 |
|---|---|
| `fixedExpenses` `variableExpenses` `cards` `categoryRules` | 0 |
| `incomeTaxes` `vatTaxes` `withholdingTaxes` | 0 |
| `config` `recipes` | 0 |

`config/passwords` 는 `firebase-config.js`(관리자앱 전용)에서만 읽고 씁니다.
통합앱은 손익 기능도 갖고 있지 않습니다(`손익` 검색 0 건) — 그래서 "재설계"가 맞습니다.

### ⚠️ 폐기 시 반드시 지킬 것 — 데이터는 지우지 않는다

**코드 폐기 ≠ 데이터 삭제입니다.**

새 손익계산서가 **과거 비용 데이터를 필요로 할 가능성이 높습니다**
(전년 대비, 고정비 추이 등). `fixedExpenses` · `variableExpenses` · `cards` ·
세무 3 종 컬렉션은 **Firestore 에 그대로 두고 Rules 로만 정리**합니다.

### 새 손익계산서를 설계할 때 정해야 할 것

- **매출 입력** — 이미 있음. 통합앱이 매출 CF 를 직접 호출합니다
- **비용 입력** — 새로 정해야 함. 기존 방식(카드 명세서 파싱 + 자동분류)을 버리므로
  대체 입력 경로가 필요합니다. 수동 입력 / 다른 연동 / 세무사 자료 업로드 등
- **범위** — SaaS 판매 대상인지 루미 전용인지. 현재는 `DECISIONS.md` 질문 1 에 따라
  루미 전용(`FINANCE_ENABLED`)이지만, 재설계본은 판매 가능성을 다시 볼 여지가 있습니다
...
| 개인 사무용품 수량·배정 관리 안 함 | 위 §재고 관련 제품 결정 1 — 구매처만 관리 |
| 자산관리 모듈 미개발 | 위 §원칙 — 타겟이 10인 전후 소규모 병원 |
