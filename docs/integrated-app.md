...
- 홈 카드 `💵 매출 결산` — **로그인한 전 직원에게 노출** (`tab-dailySales`).
  마감정산은 코디(공용 staff 계정)가 매일 하는 일이라 그 계정에서 되어야 한다.
- **역할에 따라 첫 화면이 갈린다** (`_dsCanSeeSales()` = `isBizAdmin || isAdminHigh`)
  - 직원: `staff` 뷰 — 오늘 마감 카드(저장됐으면 수납 합계 한 줄) + 최근 7일
    저장/미입력 목록. 누르면 그 날 입력·수정. **월 달력·월 누계·직원별·담당의별
    집계는 보이지 않는다.**
  - 관리자: 지금까지대로 월 달력 → 일별 상세 전부
  - `_dsRender` 가 직원의 `month`/`day` 진입을 `staff` 로 되돌린다 (경로가 새지 않게)
