...
### ✨ 파인샷 (루닛 산출기) — 탭 권한으로 노출
파인샷 시술의 오늘 처방 루닛을 뽑는 계산기. **저장하는 값은 없다** — 전부 화면 안에서만
계산하고, 결과는 `차트용 기록 복사` 로 텍스트를 뽑아 Vegas 차트에 붙인다.
그래서 Firestore 컬렉션도, 규칙 추가도 없다.

- 산출식: `총 루닛 = Σ(주 부위 기본루닛 × T계수) + 추가 부위 + 단단살 보정 + 탄력 가산`
  - T계수는 핀치 두께로 결정(`<1.5→T0 ×1.0 / <2.5→T1 ×1.5 / ≤4.0→T2 ×2.0 / 초과→T3 ×2.5`)
  - 부유방·겨뒷살은 두께 없이 +1루닛씩 붙는 추가 부위
  - 오늘 진행 루닛을 권장보다 적게 넣으면 **잔여가 이월**되고, 파인주·조사시간·
    체어타임·예약 슬롯이 진행량 비율로 같이 줄어든다
- 장비 세팅값(L HP continuous·s HP 샷·kJ)은 접힌 `장비 세팅값 (내부용)` 안에만 둔다.
...
**화면 순서** (산출기 뷰)
`👤 고객 → 📍 시술 부위 → 💪 탄력 → 🔥 열감 반응 → ⏳ 열감 내성·오늘 진행량 →
오늘 처방(차트용 기록 복사) → 📏 측정 기록 → 참고 문구`
— 진료 순서대로 입력하고 바로 차트에 붙인 뒤, 측정은 마지막에 한다.
...
- 기존 `employees.visibleTabs` 를 그대로 쓴다. 새 권한 필드를 만들지 않았다.
  - 관리자앱 `index.html` > 직원 [수정] > 📱 Staff App 탭 노출 설정 > **✨ 파인샷**
  - 통합앱 STAFF 관리 > 👥 직원 [수정] 에도 같은 체크박스가 있다
- ✨ 파인샷만 체크하면 `visibleTabs=['fineshot']` 로 저장되고, 그 계정은 로그인하면
  **파인샷 탭만** 보이고 첫 화면도 파인샷이다.
- **파인샷 단독 계정에는 내정보(급여·명세서)도 붙이지 않는다.** 통합앱 편집기는 원래
  내정보를 항상 끼워 넣지만, 파인샷만 체크한 계정은 상담 기록 전용이라 예외다
  (`_onlyFineshot`). 다른 탭과 같이 체크하면 내정보는 종전대로 자동으로 붙는다.
- 탭이 하나뿐이면 하단 네비게이션도 숨긴다(`isFineshotSoloAccount()`) — 화면을
  산출기에 다 쓴다.
- 로고·`← 홈` 을 눌러도 공용 홈 대신 파인샷으로 돌아온다
  (`isFineshotOnlyAccount()` → `openFineshotTab()`).
- 파인샷은 **기본 세트가 아니다**. 그래서 "탭 전부 체크 = `visibleTabs:null`(기본값)"
...
- **업로드·검수 화면 — 매일 코디가 쓰는 화면이라 단순함이 최우선이다.**
  ...
  - **💵 마감금액 (원장님께 드릴 현금)** — 화면 맨 위, 가장 큰 숫자.
    `현금 + 현금(현금영수증) − 미수납 본부금`. 손에 잡히는 돈만 센다 —
    통장·통장(현금영수증)은 계좌로 들어와 봉투에 없으므로 뺀다.
    계산 과정을 그대로 적어 봉투 속 돈과 한 줄씩 맞출 수 있게 한다
    (`handover`/`handoverGross`/`cashOnly`/`cashReceipt`).
...
**⚠️ 환불도 두 종류다. 각자 쪽에만 쓰고 절대 섞지 않는다.**
- `환불오더` (매출집계 표의 열) = 취소된 **오더의 매출액** → **발생매출**에서 뺀다.
  `순매출 = 총매출 − 환불오더` (`netSales`).
- `※ 환불금액` (수납 표) = 그 날 통장에서 **나간 돈** → **수납**에서 뺀다.
  마감표의 `수납금액 합계` 가 이미 순액이므로 **앱에서 또 빼면 이중 차감**이다
  (8/29 실측: 합계 열 각 줄의 합 1,757,100 − 910,000 = 847,100).
`※환불금액` 을 매출에서 빼면 안 된다 — 지난달 매출을 오늘 환불한 경우 오늘 매출이
엉뚱하게 깎인다. 같은 표에서 나온 `환불오더` 라야 날짜 귀속이 맞다.

**총매출은 표 그대로 남긴다** (`totalSales`). 검산(표끼리 대조)·객단가·직원별·
담당의별 랭킹이 전부 이 값을 쓴다 — 환불오더가 누구 몫인지 표가 알려주지 않아
개인별로는 뺄 수 없다. 환불이 있는 날은 화면에 "객단가·직원별은 총매출 기준" 이라고
밝힌다. 큰 숫자(카드 헤드라인)와 달력 히트맵만 순매출을 쓴다.

**월 합산은 일별 순액을 그대로 더한다.** 월 단위로 따로 계산하지 않는다.
```
월 순매출   = Σ 일별 (총매출 − 환불오더)   = 월 총매출 − 월 환불
월 수납     = Σ 일별 수납금액 합계          (이미 환불 차감됨)
월 마감금액 = Σ 일별 마감금액
```
월 화면에는 **총매출·환불·순매출을 셋 다** 적는다. 순매출만 보이면 "이번달 왜 이렇게
적지?" 가 되고, 환불이 함께 보이면 바로 답이 나온다.
월 경계는 완벽하지 않다 — 지난달 매출을 이번달 환불하면 이번달이 과소, 지난달이
과대로 잡힌다. 원래 매출일로 소급하려면 마감표에 없는 정보가 필요하다. 환불 발생일에
빼는 것이 실무 표준이고, 환불을 따로 보여주므로 큰 건은 눈에 띈다.
