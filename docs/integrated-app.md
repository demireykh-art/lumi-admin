...
- 💵 매출 결산 권한 — 계정별로 켜고 끔 (`settings/dailySales.emails`)
- 상담 트리 편집기 진입
...
**📍 위치 권한은 필요한 계정에만 묻는다** (`_needsGps()` → `checkLocation()`).
GPS 는 출퇴근을 클리닉 범위 안에서 찍었는지 보려고만 쓴다. 그래서 아래는
브라우저 위치 권한 팝업 자체를 띄우지 않는다 — 로그인할 때마다 물어보면
쓰는 사람만 성가시다.
- **공용계정**(`role==='shared'`) — 재고·입고·비품요청만 쓰고 출퇴근 탭이 없다
- **📍 위치 체크 면제** — STAFF 관리 > 👥 직원 > [수정] 의 체크박스
  (`employees/{id}.locationExempt`). 원장·관리자처럼 범위와 무관한 계정에 쓴다
- **출퇴근 탭을 안 쓰는 계정** — `visibleTabs` 에 `attendance` 가 없는 경우
면제 계정이 출퇴근 탭을 보게 돼 있으면 GPS 줄에 `📍 위치 체크 면제 계정` 이라고
적어, 위치 확인이 왜 없는지 알 수 있게 한다.
...
- **누가 쓸지는 관리자가 계정별로 고른다** (`_dsCanSeeSales()` =
  `isBizAdmin || isAdminHigh || canDailySales`)
  - 공용계정은 **여러 개**다. `staff@` 는 재고를 모든 직원이 같이 쓰는 계정이라
    매출이 보이면 안 되고, 데스크는 매출을 써야 한다. "공용계정이면 보여준다"로는
    가를 수 없어서 **이메일 화이트리스트**(`settings/dailySales.emails`)로 켠다.
    개인 직원 계정에도 똑같이 줄 수 있다.
  - ⚙ 관리자 설정 > **💵 매출 결산 권한** 에서 계정별로 켜고 끈다
    (`loadDailySalesPerms` / `toggleDailySalesPerm` / `addDailySalesPerm`).
    목록에는 공용 계정 전부 + 이미 권한을 준 계정이 함께 나온다.
    데스크 전용 계정은 바로 위 `+ 새 공용 계정 추가` 로 `desk@…` 를 만든 뒤 켜면 된다.
  - 관리자(`bizAdmins`·`adminHigh`)는 목록과 무관하게 항상 된다.
  - **켜진 계정은 관리자와 똑같은 화면**을 본다 — 월 달력·월 누계·직원별·담당의별까지.
    권한을 두 단계(입력만 / 전부)로 나누지 않는다. 켜졌으면 전부, 아니면 홈에
    타일조차 안 보인다.
  - 꺼진 계정이 경로로 들어와도 `_dsRender` 첫 줄에서 "권한이 없습니다" 로 막는다.
  - 화이트리스트는 로그인 시 `refreshBizAdminStatus` 가 bizAdmins·adminHigh 와
    한 번에 읽는다(`Promise.all`, 추가 왕복 없음).
- **업로드·검수 화면 — 매일 코디가 쓰는 화면이라 단순함이 최우선이다.**
  ...
  - **큰 숫자로 확인** — `💳 받은 돈 (수납)`(현금·카드·기타 / 과세·부가세)와
    `🩺 진료 매출 (발생)`(오더 건수·보험진료). 코디가 실제 화면·시재와 눈으로 맞추는 값.
    **두 이름은 매일 헷갈리는 자리라 카드마다 뜻을 붙여 적는다** —
    "오늘 실제로 들어온 금액" / "오늘 진료로 생긴 금액". 둘 다 있으면 왜 다른지도
    한 줄로 덧붙인다(선수납 사용 → 매출만, 미수 → 수납만).
  ...
  - **없는 표를 "0원" 이라고 말하지 않는다.** 매출집계 표가 안 들어온 날
    ...
    (달력 칸은 매출 대신 수납액에 `수납만` 을 붙여 표시).
  - 사진 없이 직접 입력도 가능(접힌 편집표) · 기존 날짜 [✏️ 수정] 도 같은 화면
  - `📷 표를 하나씩 골라서 올리기` 는 한 장만 다시 찍을 때 쓰는 보조 수단(접힘)
- **월 뷰**: 월 이동 · 달력 히트맵(칸=일별 발생매출·건수, 수납만 올린 날은
  수납액+`수납만`) · 월 누계 카드
  (🩺 진료 매출/객단가/💳 받은 돈/보험/비보험/과세 수납금액·부가세/입력한 날) ·
...
```
Firestore 규칙은 `signedIn()`. 코디가 공용 staff 계정으로 저장해야 하기 때문이다.
바로 옆 `revenue`·`salesDetail`·`payroll` 이 이미 `signedIn()` 이라 여기만 잠가도
월별 매출은 어차피 열려 있었다. 노출은 규칙이 아니라 **앱 화면에서** 줄인다 —
직원에게는 입력 화면만 보이고 집계는 관리자에게만 보여준다.
